...
SpiralLayout.prototype.doLayout = function (coll) {
    ...
    while (vert !== null) {
        ...
        var currentRad = this.diameter(vert) / 2
        var currentSpace = space + frontRad + currentRad;
        // involute spiral
        ...
        vert.centerX = originx + x;
        vert.centerY = originy + (-y);

        originx = vert.centerX;
        originy = vert.centerY;
        ...
        var currentCount = 0.5 * row * (row + 1);
        //var nextCount = 0.5 * (row + 1) * (row + 2);
        if (count <= currentCount) {
            if (type === "right" || type === "left down") {
                type = "right";
            }
            else if (type === "left" || type === "right down") {
                type = "left";
            }
        }
        else if (currentCount < count && currentCount + 1 === count) {
            if (type === "right") {
                type = "right down";
            }
            else if (type === "left") {
