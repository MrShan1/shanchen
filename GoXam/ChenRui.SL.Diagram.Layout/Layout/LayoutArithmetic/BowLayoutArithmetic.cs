...
namespace ChenRui.SL.Diagram.Layout
{
    ...
    /// 弓形布局算法
    /// </summary>
    public class BowLayoutArithmetic : LayoutArithmetic
    {
        /// <summary>
        /// 列数
        /// </summary>
        ...
        private Size Layout(IEnumerable<ILocatable> locatables)
        {
            ...
            locatables.ForEach(l =>
            {
                ...
                l.Location = new Point((ActualCellSize.Width / 2 + (ActualCellSize.Width + Spacing.Width) * (index % columnCount) - size.Width / 2) * (c % 2 == 0 ? 1 : -1), ActualCellSize.Height / 2 + (ActualCellSize.Height + Spacing.Height) * c - size.Height / 2);
                index++;
            });
            return size;
        }
        ...
        public override void GetConfig(object o)
        {
            ...
            {
                Spacing = new Size(ConfigContext.Current.GetDouble(ConfigKeyInfo.BOW_HORIZONTAL_SPACING_KEY),  ConfigContext.Current.GetDouble(ConfigKeyInfo.BOW_VERTICAL_SPACING_KEY));
                ColumnCount = ConfigContext.Current.GetBool(ConfigKeyInfo.BOW_FIX_COLUMN_COUNT_KEY) ? ConfigContext.Current.GetInt(ConfigKeyInfo.BOW_COLUMN_COUNT_KEY) : 0;
                RowCount = ConfigContext.Current.GetBool(ConfigKeyInfo.BOW_FIX_ROW_COUNT_KEY) ? ConfigContext.Current.GetInt(ConfigKeyInfo.BOW_ROW_COUNT_KEY) : 0;
            }
        }
