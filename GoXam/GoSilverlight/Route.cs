  ...
  public class Route : FrameworkElement {  //?? if DependencyObject, cannot set Route in a Style Setter, cannot set its DataContext
    ...
    static Route() {
      ...
      LinkReshapeHandleTemplateProperty = DependencyProperty.Register("LinkReshapeHandleTemplate", typeof(DataTemplate), typeof(Route),
        ...
        // 夏晓东加入默认单元Curviness值
      DefaultCurvinessUnitValueProperty = DependencyProperty.Register("DefaultCurvinessUnitValue", typeof(double), typeof(Route),
          null);
    }
    ...
    #region 夏晓东修改加入Curviness值的默认单元值

    private double defaultCurvinessUnitValue = 30;

    public double DefaultCurvinessUnitValue
    {
        get 
        {
             double value = (double)GetValue(DefaultCurvinessUnitValueProperty);

             value = value <= 0 ? defaultCurvinessUnitValue : value;

             return value;
        }
        set { SetValue(DefaultCurvinessUnitValueProperty, value); }
    }

    // Using a DependencyProperty as the backing store for DefaultCurvinessUnitValue.  This enables animation, styling, binding, etc...
    public static readonly DependencyProperty DefaultCurvinessUnitValueProperty;
    ...
    protected virtual double ComputeCurviness()
    {
        ...
        if (Double.IsNaN(c))
        {
            ...
            {
                // 查找第一根平线
                var theZeroCurvinessLink = bundle.Links.FirstOrDefault(one => one.Route.Curviness == 0);

                // 如果有平线
                if (theZeroCurvinessLink != null)
                {
                    // 计算Curviness值的索引基值-1
                    tempIndex = bundleindex > 0 ? bundleindex - 1 : bundleindex + 1;
                }
            }

            double spacing = DefaultCurvinessUnitValue;

            int i = Math.Abs(tempIndex);

            if (i == 0 || i == 1 || i == 2) spacing = spacing * 2;
            else spacing += spacing * .5;

            c = spacing / 2 + (i - 1) / 2 * spacing;  // i.e. +/- 5/15/25/etc from center
            if (i % 2 == 0) c = -c;  // alternate sides
            if (tempIndex <= 0) c = -c;  // other direction
        }
        return c;
