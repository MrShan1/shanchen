...
namespace Northwoods.GoXam {
...

  {
    ...
    private Size printPageSize = new Size(748, 1090);
    public Size PrintPageSize
    {
        get { return printPageSize; }
        set
        {
            printPageSize = value;
        }
    ...
    public sealed class PageInfo {  // nested class
      ...
      // 20110802 夏晓东修改,加入开始结束事件
    public event EventHandler PrintBegin;
    ...
    public void Print() {
      ...
      pdoc.Print(modelname, new PrinterFallbackSettings { ForceVector = false});
    }
    ...
    private int PageCount {
      get {
        ...
        double m_width = this.PrintPageSize.Width;
        double m_height = this.PrintPageSize.Height;

        this.EffectiveBounds = new Rect(0, 0, m_width, m_height);
        this.EffectiveScale = 1;
        this.EffectivePageSize = new Size(m_width, m_height);
    ...
    private UIElement GetPage(int pageNumber) {
      ...
      tg.Children.Add(new System.Windows.Media.TranslateTransform() { X = panel.DiagramBounds.X - viewb.X, Y = panel.DiagramBounds.Y - viewb.Y });
      ...
      //  lay.Clip = new System.Windows.Media.RectangleGeometry() { Rect = new Rect(viewb.X, viewb.Y, pixsz.Width, pixsz.Height) };
      //}
