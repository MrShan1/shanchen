...
(function (window) {
    ...
    */
    VisualizationTool.prototype.isNewNodeSelected = true;

    /**
    * 视图的新增链接是否初始为选中状态 {Boolean}
    */
    VisualizationTool.prototype.isNewLinkSelected = true;
    ...
    */
    ...
    /**
    * 自动位置提供者 {AutoLocationProvider}
    */
    VisualizationTool.prototype.autoLocationProvider = new AutoLocationProvider();
    ...
    */
    VisualizationTool.prototype.createDiagram = function (divId) {
        ...
        // 配置视图的数据模型
        this.configDiagramModel();

        // 配置撤销管理器
        this.configUndoManager();

        // 配置视图的基本设置
        this.configDiagramSetting();
    ...
    */
    VisualizationTool.prototype.createIntervalTools = function () {
        // 定时更新视图元素数量
        IntervalTool.doInterval(this, this.updatePartsCount, 1500);
    ...
    */
    VisualizationTool.prototype.loadData = function (modelData) {
        ...
        var isInitial = model.nodeDataArray.length === 0 ? true : false; // 设置视图初期化标识
        ...
        // 扩展实体信息
        this.extendDataByDataType(this.entityTypes, modelData.entities, true);

        // 扩展链接信息
        this.extendDataByDataType(this.linkTypes, modelData.links, false);
        ...
        if (this.allowInitializeLayout) {
            ...
            if (isInitial) {
                ...
                var layout = new VirtualizedCircularLayout();
                layout.doLayout(list, this.diagram);
            }
    ...
    * @return {go.GraphLinksModel} 视图的数据模型
    */
    VisualizationTool.prototype.configUndoManager = function () {
        // 创建撤销恢复管理器
        ...
        // 共享同一个撤销恢复管理器
        this.diagram.model.undoManager = undoManager;
        this.diagram.virtualizedModel.undoManager = undoManager;
    };
    ...
    */
    VisualizationTool.prototype.listenChanged = function (event) {
        ...
        if (diagram.layout.isVirtualized) {
            // 延时更新视图边界
            TimeoutTool.doDelay(diagram, diagram.computeFixedBounds, 1000);
        }
    ...
    */
    VisualizationTool.prototype.listenModelChanged = function (event) {
        ...
        if (event.change === go.ChangedEvent.Transaction
            ...
            // 延时更新视图
            TimeoutTool.doDelay(this.diagram, this.diagram.updateAllTargetBindings, 1);
            //TimeoutTool.doDelay(this.diagram, this.diagram.rebuildParts, 1);
        }
    };
    ...
    */
    VisualizationTool.prototype.updateDiagramParts = function () {
        ...
        //diagram.skipsUndoManager = false; // 临时停止操作记录功能，不在操作历史中记录该处理过程
    ...
    */
    VisualizationTool.prototype.removePartsOutOfView = function () {
        ...
        while (iterator.next()) {
            ...
            if (!node.isSelected && !node.isHighlighted && !node.actualBounds.intersectsRect(viewportBounds)) {
    ...
    */
    VisualizationTool.prototype.editLinkType = function (dataType) {
        ...
        // 设置是否为双向线（双向线无箭头，单向线有箭头）
        dataType.isTwoWay = dataType.isTwoWay ? true : false;
    };
    ...
    * 过滤重复数据
    *
    * @param array {Array} 需要过滤的数据集合
    * @return {Array} 过滤后的数据集合
    */
    VisualizationTool.prototype.filterArray = function (array) {
        if (!array || array.length === 0) return [];

        var map = new go.Map("string", "object");
        var resultArray = [];

        for (var i = 0; i < array.length; i++) {
            var item = array[i];
            var mapItem = map.getValue(item.id);

            if (!mapItem || (!mapItem.isMain && item.isMain)) {
                map.add(item.id, item);
            }
        }

        map.iteratorValues.each(function (data) {
    ...
    */
    VisualizationTool.prototype.editMainEntityProperty = function (dataType, data) {
        ...
        // 设置图片路径
        data.iconName = data.iconName || dataType.iconName;
        // 设置显示状态
        ...
        // 初始化节点的图形边界值(参考值：（0, 0, 230, 150）)
        data.bounds = data.boundString ? go.Rect.parse(data.boundString) : new go.Rect(0, 0, 133, 133);
        // 初始化是否为虚拟状态标识（是否在视图中）
        data.isVirtual = true;
    };
    ...
    */
    VisualizationTool.prototype.editMainLinkProperty = function (dataType, data) {
        ...
        data.showTextList = false;
        // 设置是否为双向线（箭头显示状态）
        data.isTwoWay = data.isTwoWay === undefined ? dataType.isTwoWay : data.isTwoWay;
        // 初始化是否为虚拟状态标识（是否在视图中）
        data.isVirtual = true;
    };
    ...
    */
    VisualizationTool.prototype.saveDataToJson = function () {
        ...
        entities.forEach(function (data) {
            if (data.bounds) {
                data.boundString = go.Rect.stringify(data.bounds);
            }
        });
    ...
    */
    MessageTool.ShowSuccessMessage = function (msg) {
        alert(msg);
    };
    ...
    * @param msg {String} 消息字符串
    */
    MessageTool.ShowErrorMessage = function (msg) {
        alert(msg);
    };
    ...
    * @param msg {String} 消息字符串
    */
    MessageTool.ShowWarningMessage = function (msg) {
        alert(msg);
    };
    ...
    */
    TemplateManager.prototype.styles = {
        ...
        textBlockStyle: {
            ...
            maxSize: new go.Size(250, 30),
            //maxSize: new go.Size(450, 50),
    ...
    */
    TemplateManager.prototype.convertBoundsByPosition = function (data, objectData) {
        ...
        return new go.Rect(data.x, data.y, objectData.bounds.width, objectData.bounds.height);
    ...
    * 执行布局（override）
    *
    * @param coll {go.Iterable|Array} 数据集合
    * @param diagram {go.Diagram} 数据所属视图
    */
    VirtualizedTreeLayout.prototype.doLayout = function (coll, diagram) {
        if (coll && (coll.iterator || coll instanceof Array)) {
            if (!diagram) return;

            this.diagram = diagram;
        }

        go.TreeLayout.prototype.doLayout.apply(this, arguments);
    };
    ...
    */
    VirtualizedTreeLayout.prototype.makeNetwork = function (coll) {
        ...
        if (coll && coll.iterator) {
            coll.each(function (data) {
                data.isNode ? nodeDataArray.push(data) : linkDataArray.push(data);
            });
        }
        else if (coll && coll instanceof Array) {
            coll.forEach(function (data) {
                data.isNode ? nodeDataArray.push(data) : linkDataArray.push(data);
            });
        }
        else {
            nodeDataArray = this.diagram.currentModel.nodeDataArray;
            linkDataArray = this.diagram.currentModel.linkDataArray;
        }

        // 添加虚拟模型数据
        network.addVirtualizedModel(nodeDataArray, linkDataArray);
    ...
    * @param diagram {go.Diagram} 数据所属视图
    */
    VirtualizedCircularLayout.prototype.doLayout = function (coll, diagram) {
        if (coll && (coll.iterator || coll instanceof Array)) {
            if (!diagram) return;
        ...
        go.CircularLayout.prototype.doLayout.apply(this, arguments);
    };
    ...
    * @param diagram {go.Diagram} 数据所属视图
    */
    VirtualizedForceDirectedLayout.prototype.doLayout = function (coll, diagram) {
        if (coll && (coll.iterator || coll instanceof Array)) {
            if (!diagram) return;
        ...
        go.ForceDirectedLayout.prototype.doLayout.apply(this, arguments);
    };
    ...
    */
    VirtualizedForceDirectedLayout.prototype.makeNetwork = function (coll) {
        ...
        // 节点坐标回归初始位置，不根据当前位置微调
        nodeDataArray.forEach(function (data) {
            if (data.bounds && data.isVisible) {
                data.bounds = new go.Rect(0, 0, data.bounds.width, data.bounds.height);
            }
        });
    ...
    */
    VirtualizedLayeredDigraphLayout.prototype.makeNetwork = function (coll) {
        ...
        var network = this.createNetwork();
        // 添加虚拟模型数据
        network.addVirtualizedModel(this.diagram.currentModel);
    ...
    */
    ...
    /**
    * 是否为平衡布局 {Boolean} 布局尽量接近于方形
    */
    VirtualizedGridLayout.prototype.isBalanced = true;
    ...
    * @param diagram {go.Diagram} 数据所属视图
    */
    VirtualizedGridLayout.prototype.doLayout = function (coll, diagram) {
        if (!coll) return;
        ...
        var startPointX = Infinity; // 布局开始位置的横坐标
        var startPointY = Infinity; // 布局开始位置的纵坐标
        var spaceW = this.spacing.width; // 节点横向间隔距离
        var spaceH = this.spacing.height; // 节点纵向间隔距离

        vertexes.each(function (vertex) {
            var bounds = vertex.data.bounds;

            // 更新布局开始位置的横坐标
            if (bounds.x < startPointX) startPointX = bounds.x;

            // 更新布局开始位置的纵坐标
            if (bounds.y < startPointY) startPointY = bounds.y;

            // 更新节点横向间隔距离
            if (spaceW < bounds.width) spaceW = bounds.width;

            // 更新节点纵向间隔距离
            if (spaceH < bounds.height) spaceH = bounds.height;
        });

        var count = vertexes.count;
        var wrappingColumn = Math.ceil(Math.sqrt(count)); // 获取布局列数
        var index = 0;
        var iterator = vertexes.iterator;

        while (iterator.next()) {
            var vertex = iterator.value;
            var col = index % wrappingColumn; // 获取该节点所在列
            var row = Math.floor(index / wrappingColumn); // 获取该节点所在行
            var bounds = vertex.data.bounds;
            var x = startPointX + spaceW * col; // 获取节点的中心点位置横坐标
            var y = startPointY + spaceH * row; // 获取节点的中心点位置纵坐标

            if (this.alignment === go.GridLayout.Position) {
                x = x + bounds.width / 2;
                y = y + bounds.height / 2;
            }
    ...
    ToolbarManager.prototype.addControllerForTool = function (controller, targetTool) {
        ...
        eventListener = (function (controller) {
            return function () {
                ...
                var value = converter ? converter(listenerParam, targetTool) : listenerParam;
    ...
    */
    ColorConfigTool.prototype.doExcute = function (color) {
        ...
        var nodeDatas = model.filterNode(function (data) {
            if (data.isNode && data.isSelected && data.isVisible) {
                return true;
            }
        ...
        if (nodeDatas.count > 0) {
            nodeDatas.each(function (data) {
                model.setDataProperty(data, "color", color);
            });
        } else {
            MessageTool.ShowWarningMessage("请选择一个或多个节点");
        }
    ...
    */
    LayoutConfigTool.prototype.doExcute = function (layoutName) {
        ...
        // 为虚拟化布局更新视图内容
        if (this.visualizationTool.allowVirtualize) {
            this.visualizationTool.updateDiagramParts();
        }
        ...
        // 视图居中
        this.diagram.centerRect(this.diagram.fixedBounds);
    ...
    */
    SelectionConfigTool.prototype.doExcute = function (selectWay) {
        ...
        switch (selectWay) {
            // 选择全部节点
            case vis.SelectionConfigTool.AllNodes:
                model.eachNode(function (data) {
                ...
                // 选择全部链接
            case vis.SelectionConfigTool.AllLinks:
                model.eachLink(function (data) {
                    if (data.isVisible) {
                        model.setDataProperty(data, "isSelected", true);
                    }
                ...
                // 选择同类型节点
            case vis.SelectionConfigTool.SameTypeNodes:
                ...
                // 选择同类型链接
            case vis.SelectionConfigTool.SameTypeLinks:
                var types = new go.Set("string");
                ...
                if (types.count > 0) {
                    ...
                    MessageTool.ShowWarningMessage("请选择一个或多个链接");
                }

                break;

                // 选择节点直接关系
            case vis.SelectionConfigTool.NodeRelations:
                ...
                // 选择链接直接关系
            case vis.SelectionConfigTool.LinkRelations:
                var selection = new go.List("object");
                ...
                // 反向选择节点
            case vis.SelectionConfigTool.UnselectedNodes:
                model.eachNode(function (data) {
                    if (data.isVisible && data.isNode) {
                ...
                // 反向选择链接
            case vis.SelectionConfigTool.UnselectedLinks:
                model.eachLink(function (data) {
                    if (data.isVisible) {
                        model.setDataProperty(data, "isSelected", !data.isSelected);
                    }
                ...
                // 选择全部节点和链接
            case vis.SelectionConfigTool.All:
                model.eachNode(function (data) {
                    if (data.isVisible && data.isNode) {
                        model.setDataProperty(data, "isSelected", true);
                    }
                ...
                // 反向选择节点和链接
            case vis.SelectionConfigTool.Unselected:
                model.eachNode(function (data) {
                    if (data.isVisible && data.isNode) {
                        model.setDataProperty(data, "isSelected", !data.isSelected);
                    }
            ...
            if (data.isSelected) {
                ...
                if (data.isNode) {
                    ...
                    links.each(function (linkData) {
                        model.setDataProperty(linkData, "isVisible", false);
                        model.setDataProperty(linkData, "isSelected", false);
                    });
                }
            }
            else {
    ...
    */
    ShowHiddenPartsTool.prototype.doExcute = function () {
        ...
        var model = this.diagram.currentModel;
        var hiddenNodeDatas = model.filterNode(function (data) {
    ...
    */
    CenterSelectionTool.prototype.doExcute = function () {
        ...
        model.eachLink(function (data) {
            if (data.isSelected) {
                var fromNodeData = model.findNodeDataForKey(data.fromEntityId);
                if (fromNodeData) {
                    nodeDataList.add(fromNodeData);
                }

                var toNodeData = model.findNodeDataForKey(data.toEntityId);
                if (toNodeData) {
                    nodeDataList.add(toNodeData);
                }
            }
        });
        ...
        if (nodeDataList.count > 0) {
            ...
            // 计算矩形边界
            nodeDataList.each(function (data) {
                if (data.bounds) {
                    if (rect) {
                        rect.unionRect(data.bounds);
                    }
                    else {
                        rect = new go.Rect()
                        rect.set(data.bounds);
                    }
                }
            });
            ...
            this.diagram.zoomToRect(new go.Rect(rect.x - 300, rect.y - 300, rect.width + 600, rect.height + 600));
        } else {
            MessageTool.ShowWarningMessage("请选择一个或多个视图元素进行定位");
        }
    ...
    */
    InitializeLayoutTool.prototype.doExcute = function () {
        ...
        //this.diagram.skipsUndoManager = true; // 临时停止操作记录功能，不在操作历史中记录该处理过程
        ...
        //this.diagram.skipsUndoManager = oldskips; // 恢复操作记录功能
    };
    ...
    */
    SearchTextTool.prototype.isNodeDataMatched = function (data, condition) {
    ...
    * @return {Boolean} 是否匹配标识
    */
    SearchTextTool.prototype.isLinkDataMatched = function (data, condition) {
        if (!data.isVisible || !data.propertyValues) return false;
    ...
    */
    SearchTextTool.prototype.doExcute = function (text) {
        ...
        if (!text) {
            ...
            MessageTool.ShowWarningMessage("检索条件不能为空");
            return;
        }
            // 检索内容改变时，重新检索
            ...
            for (var i = 0; i < nodeDataArrayLength; i++) {
                ...
                // 添加匹配数据
                if (this.isNodeDataMatched(data, this.currentText)) {
            ...
            for (var i = 0; i < linkDataArrayLength; i++) {
                ...
                // 添加匹配数据
                if (this.isLinkDataMatched(data, this.currentText)) {
                    this.searchResults.push(data);
                }
            }
        ...
        if (this.searchResults.length > 0) {
            ...
            MessageTool.ShowWarningMessage("未检索到结果");
            return;
        }
    };
            ...
            if (object !== null) {
                ...
                if (count < 2 && object instanceof go.Node) {
                    if (!object.isSelected) {
                        if (count = 0 || !analysisPathTool.startNodeData) {
    ...
    */
    go.Diagram.prototype.computeFixedBounds = function () {
        ...
        this.nodes.each(function (obj) {
            ...
            // 更新节点的宽高数据
            model.setDataProperty(obj.data, "bounds", new go.Rect(obj.data.bounds.x, obj.data.bounds.y, obj.actualBounds.width, obj.actualBounds.height));
        });
        ...
        model.eachNode(function (node) {
            ...
            if (node.group) {
                ...
                if (!groupData || (groupData && groupData.isVisible)) {
                    ...
                    unionBounds = new go.Rect(node.bounds.x - 100, node.bounds.y - 100,
                        node.bounds.width + 200, node.bounds.height + 200);
                }
            } else {
                unionBounds = node.bounds;
            }
    ...
    * 添加虚拟模型数据
    *
    * @param nodeDataArray {Array} 节点数据集合
    * @param linkDataArray {Array} 链接数据集合
    */
    go.LayoutNetwork.prototype.addVirtualizedModel = function (nodeDataArray, linkDataArray) {
        ...
        for (var i = 0; i < linkLength; i++) {
            ...
            // 两端节点不存在或不可见时，跳过
            if (!fromVertex || !fromVertex.data.isVisible || !toVertex || !toVertex.data.isVisible) continue;
    ...
    */
    go.GraphLinksModel.prototype.findNodesConnected = function (nodeData) {
        ...
        this.eachLink(function (linkData) {
            ...
            if (otherNode && otherNode.isVisible && !list.contains(otherNode)) {
                list.add(otherNode);
            }
        });
    ...
    * @param linkData {Object} 链接数据
    */
    VirtualizedModel.prototype.removeLinkData = function (linkData) {
        if (this.diagramModel) {
            var diagramData = this.diagramModel.findLinkDataForKey(linkData.id);
            if (diagramData) {
                go.GraphLinksModel.prototype.removeLinkData.apply(this.diagramModel, arguments);
            }
        }

        go.GraphLinksModel.prototype.removeLinkData.apply(this, arguments);
    ...
    * @param coll {go.Iterable|Array} 数据集合
    */
    VirtualizedModel.prototype.removeLinkDataCollection = function (coll) {
        if (!coll) return;
    ...
    * @param nodeData {Object} 节点数据
    */
    VirtualizedModel.prototype.removeNodeData = function (nodeData) {
        if (this.diagramModel) {
            var diagramData = this.diagramModel.findNodeDataForKey(nodeData.id);
            if (diagramData) {
                go.GraphLinksModel.prototype.removeNodeData.apply(this.diagramModel, arguments);
            }
        }

        go.GraphLinksModel.prototype.removeNodeData.apply(this, arguments);
    };
    ...
    * @param coll {go.Iterable|Array} 数据集合
    */
    VirtualizedModel.prototype.removeNodeDataCollection = function (coll) {
        if (!coll) return;
    ...
    */
    VirtualizedModel.prototype.setDataProperty = function (data, propertyName, value) {
        ...
        if (this.diagramModel) {
            diagramData = this.diagramModel.findNodeDataForKey(data.id);
            if (!diagramData) {
                diagramData = this.diagramModel.findLinkDataForKey(data.id);
            }
        }
    ...
    * 移动间距 {Number}
    */
    AutoLocationProvider.prototype.moveDistance = 200;

    /**
    * 扫描角度 {Number} 最大环绕一周（360度）
    */
    AutoLocationProvider.prototype.sweepAngle = 180;

    /**
    * 计算坐标点
    *
    * @param centerPoint {go.Point} 原点坐标
    * @param radius {Number} 坐标点与原点的距离
    * @param angle {Number} 坐标点与原点在x轴方向的角度
    * @return {go.Point} 坐标点
    */
    AutoLocationProvider.prototype.computePoint = function (centerPoint, distance, angle) {
        var x = centerPoint.x + distance * Math.cos(angle / 180 * Math.PI);
        var y = centerPoint.y + distance * Math.sin(angle / 180 * Math.PI);
    ...
    * 获取节点数据集合的中心点
    *
    * @param nodeDataList {go.List} 节点数据集合
    * @return {go.Point} 中心点
    */
    AutoLocationProvider.prototype.getCenterPoint = function (nodeDataList) {
        var centerPoint = new go.Point(0, 0); // 默认中心点为原点

        // 获取节点数据所在的矩形
        var rect = this.getRect(nodeDataList);
    ...
    * 获取位置
    *
    * @param mainNodeDataList {go.List} 主要节点数据集合
    * @param rect {go.Diagram} 目标节点数据集合所在的矩形边界
    * @param startPoint {go.Point} 起始点
    * @param startAngle {Number} 起始角度
    * @param startRadius {Number} 起始半径
    * @return {go.Point} 最终位置
    */
    AutoLocationProvider.prototype.getLocation = function (mainNodeDataList, rect, startPoint, startAngle, startRadius) {
        var location = null;
        var index = 0;

        // 循环扫描，直至找到空白位置为止
        while (!location) {
            var distance = this.moveDistance; // 获取偏移距离
            var radius = startRadius + distance * index;  // 获取扫描半径
            //var rotation = distance / (Math.PI * radius) * 180; // 将偏移距离作为圆周弧线，获取偏移距离对应的夹角角度
            var rotation = Math.asin(distance / 2 / radius) * 180 / Math.PI; // 获取偏移距离对应的夹角角度
            var sweep = this.sweepAngle / 2; // 获取与起始角度的最大偏移量

            for (var offset = 0; offset < sweep; offset = offset + rotation) {
                var newRect = rect.copy();

                // 正向偏移
                newRect.center = this.computePoint(startPoint, radius, startAngle + offset);
                // 确认新区域是否为空白区域
                if (!this.hasNodeDataAtRect(mainNodeDataList, newRect)) {
                    location = newRect.position;
                    break;
                }
    ...
    /**
    * 获取主要节点数据集合（目标数据集合之外的节点数据）
    *
    * @param diagram {go.Diagram} 主视图
    * @param targetNodeDataList {go.List} 要设置位置的节点数据集合
    * @return {go.List} 主视图节点数据集合
    */
    AutoLocationProvider.prototype.getMainNodeDataList = function (model, targetNodeDataList) {
        var list = model.filterNode(function (data) {
            if (data.isNode && !targetNodeDataList.contains(data)) {
                return true;
            }
    ...
    *
    * @param nodeDataList {go.List} 节点数据集合
    * @return {go.Rect} 节点数据所在的矩形边界
    */
    AutoLocationProvider.prototype.getRect = function (nodeDataList) {
        var rect = null;
    ...
    * 获取起始半径
    *
    * @param rect {go.Rect} 目标矩形
    * @return {Number}起始半径
    */
    AutoLocationProvider.prototype.getStartRadius = function (rect) {
        var w = rect.width;
        var h = rect.height;
        var radius = Math.sqrt(w * w + h * h) / 2; // 矩形最小外接圆半径
    ...
    * @param nodeDataList {go.List} 节点数据集合
    * @param rect {go.Rect} 目标矩形
    * @return {Boolean} 存在节点数据标识
    */
    AutoLocationProvider.prototype.hasNodeDataAtRect = function (nodeDataList, rect) {
        var hasNodeDataAtRect = false;
        var iterator = nodeDataList.iterator;

        // 判定目标矩形内是否存在节点数据
        while (iterator.next()) {
            var nodeData = iterator.value;
            if (nodeData.bounds && rect.intersectsRect(nodeData.bounds)) {
                hasNodeDataAtRect = true;
                break;
            }
        }

        return hasNodeDataAtRect;
    };
    ...
    *
    * @param model {go.GraphLinksModel} 数据模型
    * @param nodeDataList {go.List} 目标节点数据集合
    * @param fromPoint {go.Point} 主视图
    * @param toPoint {go.Point} 起始点
    */
    AutoLocationProvider.prototype.moveNodeDataList = function (model, nodeDataList, fromPoint, toPoint) {
        ...
        nodeDataList.each(function (data) {
            if (data.bounds) {
                var rect = data.bounds.copy().offset(dx, dy)
                model.setDataProperty(data, "bounds", rect);
            }
        });
    };
    ...
    *
    * @param diagram {go.Diagram} 主视图
    * @param nodeDataList {go.List} 要设定位置的目标节点数据集合
    * @param startPoint {go.Point} 设定位置处理的起始坐标
    */
    AutoLocationProvider.prototype.setLocation = function (diagram, targetNodeDataList, startPoint) {
        if (!diagram || !targetNodeDataList || targetNodeDataList.count === 0) return;

        var model = diagram.currentModel;

        // 获取主要节点数据集合
        var mainNodeDataList = this.getMainNodeDataList(model, targetNodeDataList);

        // 若视图上的节点均在目标节点集合中，则不需要计算位置
        if (mainNodeDataList.count === 0) {
            return;
        }

        // 获取主要节点数据集合的中心点
        var centerPoint = this.getCenterPoint(mainNodeDataList);
        ...
        if (!startPoint) {
            ...
            // test
            if (diagram.startNodes && diagram.startNodes.count > 0) {
                startPoint = this.getCenterPoint(diagram.startNodes);
                diagram.startNodes = null;
            }
        }

        // 获取目标节点数据集合的矩形边界
        var rect = this.getRect(targetNodeDataList);

        // 获取以起始点为中心的矩形边界
        var startPointRect = rect.copy();
        startPointRect.center = startPoint;

        // 若起始点所在的矩形边界非空白区域，则自动寻找位置
        if (this.hasNodeDataAtRect(mainNodeDataList, startPointRect)) {
            // 获取目标点与中心点的角度
            var startAngle = centerPoint.directionPoint(startPoint);

            // 获取起始扫描半径
            var startRadius = this.getStartRadius(rect);

            // 获取最终位置
            var finalLocation = this.getLocation(mainNodeDataList, rect, startPoint, startAngle, startRadius);

            // 将目标节点数据集合移动至最终位置
            this.moveNodeDataList(model, targetNodeDataList, rect, finalLocation);
        }
            // 若起始点所在的矩形边界是空白区域，则直接将目标节点数据集合移动至起始点
        else {
            // 将目标节点数据集合移动至最终位置
            this.moveNodeDataList(model, targetNodeDataList, rect.center, startPoint);
        }
    };
    ...
    var vis = {
        ...
        CenterSelectionTool: CenterSelectionTool,
        SearchTextTool: SearchTextTool,
        ShowMagnifierTool: ShowMagnifierTool,
        AnalysisPathTool: AnalysisPathTool,
        VirtualizedModel: VirtualizedModel,
        CustomNode: CustomNode,
        ParallelLink: ParallelLink,
        VirtualizedCircularLayout: VirtualizedCircularLayout,
        VirtualizedForceDirectedLayout: VirtualizedForceDirectedLayout,
        VirtualizedGridLayout: VirtualizedGridLayout
    };
    window.vis = vis;
